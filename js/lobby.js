...
function hideAllMenus() {
    const menus = ['mainMenu', 'roomListModal', 'lobbyModal', 'mapSelectMenu', 'towerSelectModal', 'shopModal', 'authScreen'];
    menus.forEach(id => {
        ...
        startBtn.addEventListener('click', async () => {
            try {
                ...
                if (candidates.length === 0) {
                    ...
                    const topMaps = Object.keys(voteCounts).filter(m => voteCounts[m] === maxCount);
                    if (topMaps.length > 1) {
                        selectedMap = topMaps[Math.floor(Math.random() * topMaps.length)];
                    }
                }
                ...
                // Удаляем голоса, чтобы не занимали место
                await supabase
                    .from('room_votes')
                    .delete()
                    .eq('room_id', room.id);
